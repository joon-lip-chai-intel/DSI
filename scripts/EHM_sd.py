...
def build_dict(raw_dlog_fn):
    ...
    for line in ipf:
        if "ITUFF_PORT_STATUS##" in line:
            dlog_dict["PORT_STATUS"] = ('str', line.replace('##ITUFF_PORT_STATUS##','').strip("\n"))
...
if __name__ == '__main__':
    try:
        ...
        if len(sys.argv) != 4:
            ...
            print("Expected: python.exe EHM_sd.py <test name> <raw datalog file path> <structure datalog file path>")
            sys.exit(1)
