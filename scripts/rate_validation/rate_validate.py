...
def run():
        ...
        f = open("//fs3//Ocelot//DSI//bin//bit_rate.txt", "r")
        found_port = 0
        found_port2x = 0
        for line in f:
            for i in port_range:
        ...
        print (f"{ituff_limit}")
        print (f"{ituff_value}")
        with open ("//fs3//Ocelot//DSI//bin//ituff_logging.txt", "w") as output_file:
        ...
        if args.linerate_snake > 0:
            ...
            port_up_default = 4 #checking inline traffic only, ignore the other ports, require 4 ports up
