...
##		- test only the 50G port on each quad								 #
##		- refer to the original test _TTR_100G for full test				 #
##############################################################################
"""
...
print(f"#DSI_START#Checking for {args.port_speed_set}G {args.traffic} single lane traffic on Quad starting with Port{args.quad} with minimum linerate expected: {args.linerate_traffic_single}%")
...
def run():
		...
		for i in range(args.quad,args.quad + portpergroup):
			#logging.info("##The linerate for port", i, "operating at", args.port_speed_set,"G is", port_dict['line_rate{0}'.format(i)])
			print(f"##The linerate for port{i} operating at 50G is {port_dict['line_rate{0}'.format(i)]}%")
		#print for port2x
		for i in port2x_range:
			#logging.info("##The linerate for port", i, "operating at 100G is", port_dict['line_rate{0}'.format(i)])
		...
		if args.linerate_snake > 0:
			...
			port_up_default = portpergroup #checking inline traffic only, ignore the other ports, require the 2 50G ports up (0 1 2 3 - ports 2 and 3 need to meet 50G in a multilane setup, 100G is tested separately)
		...
		if Fail==0:
			print (f"##The linerate for Quad{args.quad} is above {line_rate_limit_traffic_single}% for {args.traffic} traffic and test Passed.#DSI_END#")
			exit(0)
		else:
			print (f"##The linerate for Quad{args.quad} is below the threshold level of {line_rate_limit_traffic_single}% and test Failed.#DSI_END#")
			exit(1)
