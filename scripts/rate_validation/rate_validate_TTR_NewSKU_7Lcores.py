...
## Updated by: taipinnt 250903												 #
##		- simplified looping using port per group							 #
##		- refer to the original test _TTR for original test					 #
##############################################################################
"""
...
parser.add_argument("-t_lr", "--linerate_traffic", help="select what linerate for traffic to verify against.", default="30",type=int)
...
print(f"#DSI_START#Checking for {args.port_speed_set}G {args.traffic} traffic on Quad starting with Port{args.quad} with minimum linerate expected: {args.linerate_traffic}%")

def run():
	
		"""
		mapping of the ports are in order of port per group
		port per group = 2
		quad 0 : ports 0 till 1
		quad 2 : ports 2 till 3
		...
		...
		quad 16: ports 16 till 17
		quad 18: ports 18 till 19
		...
		if args.port_speed_set==25:
			args.phy="CR"
		#50G is pushed in, no need set, in fact can't hardcode.

		# use of range() to define a range of values
		port_range = range(20)
		port2x_range = range(20,23)
		port_dict = {}
		portpergroup = 2 #number of ports tested in each iteration
		...
		for i in range(args.quad,args.quad + portpergroup):
			#logging.info("##The linerate for port", i, "operating at", args.port_speed_set,"G is", port_dict['line_rate{0}'.format(i)])
			print(f"##The linerate for port{i} operating at {args.port_speed_set}G is {port_dict['line_rate{0}'.format(i)]}%")
		#print for port2x
		for i in port2x_range:
			#logging.info("##The linerate for port", i, "operating at 100G is", port_dict['line_rate{0}'.format(i)])
		...
		if args.quad==0:
			for j in range (0,args.quad + portpergroup):
				linerate_limit.append(str(args.linerate_traffic))
			for k in range (args.quad + portpergroup,20):
				linerate_limit.append(str(args.linerate_snake))
		elif args.quad==(20-portpergroup):
			for k in range (0,args.quad):
				linerate_limit.append(str(args.linerate_snake))
			for j in range (args.quad,20):
				linerate_limit.append(str(args.linerate_traffic))		   
		else:
			for k in range (0,args.quad):
				linerate_limit.append(str(args.linerate_snake))
			for j in range (args.quad,args.quad + portpergroup):
				linerate_limit.append(str(args.linerate_traffic))
			for k in range (args.quad + portpergroup,20):
				linerate_limit.append(str(args.linerate_snake))
		...
		if args.linerate_snake > 0:
			...
			port_up_default = portpergroup #checking inline traffic only, ignore the other ports, require 4 ports up #2 ports up for new 8c sku
		...
		line_rate_limit_traffic = args.linerate_traffic
		...
		if args.quad==0:
			for j in range (0,args.quad + portpergroup):
				if not port_dict['line_rate{0}'.format(j)] >= line_rate_limit_traffic:
					Fail = Fail + 1
			for k in range (args.quad + portpergroup,20):
				if not port_dict['line_rate{0}'.format(k)] >= line_rate_limit_snake:
					Fail = Fail + 1
		elif args.quad==(20-portpergroup):
			for j in range (20-portpergroup,20):
				if not port_dict['line_rate{0}'.format(j)] >= line_rate_limit_traffic:
					Fail = Fail + 1
			for k in range (0,args.quad):
				if not port_dict['line_rate{0}'.format(k)] >= line_rate_limit_snake:
		...
		# Check for any failed ports
		if Fail==0:
			print (f"##The linerate for Quad{args.quad} is above {line_rate_limit_traffic}% for {args.traffic} traffic and test Passed.#DSI_END#")
			exit(0)
		else:
			print (f"##The linerate for Quad{args.quad} is below the threshold level of {line_rate_limit_traffic}% and test Failed.#DSI_END#")
			exit(1)
