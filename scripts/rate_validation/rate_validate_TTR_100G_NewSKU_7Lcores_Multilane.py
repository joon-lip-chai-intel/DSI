...
##		- test only the 100G port on each quad								 #
...
def run():
		...
		port_dict = {}
		portpergroup = 2 #number of ports tested in each iteration, not used in this test, only for single lane
		...
		#logging.info("##The linerate for port", i, "operating at", args.port_speed_set,"G is", port_dict['line_rate{0}'.format(i)])
		print(f"##The Multi-lane linerate for port{args.quad} operating at {args.port_speed_set}G is {port_dict['line_rate{0}'.format(i)]}%")
		...
		if args.linerate_snake > 0:
			...
			port_up_default = 1 #checking inline traffic only, ignore the other ports, require 1 multilane port up
		...
		# Port 0-19
		if not port_dict['line_rate{0}'.format(args.quad)] >= line_rate_limit_traffic_multi:
			Fail = Fail + 1
		...
		if Fail==0:
			print (f"##The 100G Multi-lane linerate for Quad{args.quad} is above {line_rate_limit_traffic_multi}% for {args.traffic} traffic and test Passed.#DSI_END#")
			exit(0)
		else:
			print (f"##The 100G Multi-lane linerate for Quad{args.quad} is below the threshold level of {line_rate_limit_traffic_multi}% and test Failed.#DSI_END#")
			exit(1)
